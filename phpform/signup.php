...
if(isset($_POST['save'])){   
	...
	$Student_Emailid=$_POST['email'];
	$PassWord=$_POST['password'];
    $Username=$_POST['username'];
    $Student_Gender=$_POST['gender'];


// validation for duplicate student name and email
echo "<br>".$sqlvalidation = "select * from tblsignupform where email_id='".$Student_Emailid."' and Password='".$PassWord."'" ;
$resvalidation=mysql_query($sqlvalidation);
echo "<br>Cnt---0".$Cnt=mysql_num_rows($resvalidation);

if($Cnt>0){
	echo"<script> alert('Student Name is Already Exits....');window.location.assign('signup.php') </script>";
}
else{
// insert record
	echo "<br>". $SqlStudent="insert into tblsignupform(Firstname,Lastname,email_id,Password,UserName,Gender) values('".$Student_Fname."','".$Student_Lname."','".$Student_Emailid."','".$PassWord."','". $Username."','".$Student_Gender."')";
    $ResStudent=mysql_query($SqlStudent);
    $studentId=mysql_insert_id();
    $insloginsql="insert into tblloginform(UserName,PassWord,idStudent) values('".$Username."','".$PassWord."','".$studentId."')";
    $reslogin=mysql_query($insloginsql);
    echo "<script> alert('Student Information Added Successfully');window.location.assign('main.php') </script>";
}
